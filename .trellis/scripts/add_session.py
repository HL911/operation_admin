...
> 承接自 `{FILE_JOURNAL_PREFIX}{prev_num}.md`（约 {max_lines} 行时归档）
> 开始时间：{today}
...
def update_index(
    ...
    if "@@@auto:current-status" not in content:
        print("错误：index.md 中未找到自动标记，请确认标记仍然存在。", file=sys.stderr)
        return False
...
def add_session(
    ...
    if target_file:
        ...
        print(f"[OK] 已将会话追加到 {target_file.name}", file=sys.stderr)
